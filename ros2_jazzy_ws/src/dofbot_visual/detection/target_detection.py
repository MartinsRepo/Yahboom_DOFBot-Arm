...
with open(os.path.join(BASE_DIR, 'object_detection_coco.txt'), 'r') as f:
    class_names = f.read().split('\n')
# get a different color array for each of the classes
COLORS = np.random.uniform(0, 255, size=(len(class_names), 3))
# load the DNN modelimage
model = cv.dnn.readNet(
    model=os.path.join(BASE_DIR, 'frozen_inference_graph.pb'),
    config=os.path.join(BASE_DIR, 'ssd_mobilenet_v2_coco.txt'),
...
net = cv.dnn.readNetFromTensorflow(os.path.join(BASE_DIR, "graph_opt.pb"))
