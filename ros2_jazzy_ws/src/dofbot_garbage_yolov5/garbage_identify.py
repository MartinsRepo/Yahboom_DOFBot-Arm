...
def _resolve_model_path():
    # Allow explicit override, then try local package paths.
    env_path = os.environ.get('DOFBOT_GARBAGE_MODEL')
    if env_path and os.path.exists(env_path):
        return env_path

    base_dir = os.path.dirname(os.path.abspath(__file__))
    candidates = [
        os.path.join(base_dir, 'model0.pt'),
        os.path.join(base_dir, 'model.pt'),
        os.path.join(base_dir, 'models', 'model0.pt'),
        os.path.join(base_dir, 'models', 'model.pt'),
    ]
    for candidate in candidates:
        if os.path.exists(candidate):
            return candidate
    raise FileNotFoundError('No YOLO model file found. Set DOFBOT_GARBAGE_MODEL or place model0.pt in package directory.')
...
class garbage_identify:
    ...
    def server_joint(self, posxy):
        ...
        if not self.client.wait_for_service(timeout_sec=2.0):
            raise RuntimeError('dofbot_kinemarics service is not available')
        ...
        try:
            ...
            self.node.get_logger().info("arg error")
