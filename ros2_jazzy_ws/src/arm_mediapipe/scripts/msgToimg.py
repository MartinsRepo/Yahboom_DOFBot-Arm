...
from yahboomcar_msgs.msg import ImageMsg
...
class msgToimg:
    def __init__(self):
        ...
        self.image_sub = rospy.Subscriber("/image_msg", ImageMsg, self.image_sub_callback)
        self.pub_img = rospy.Publisher("/image", Image, queue_size=10)

    def image_sub_callback(self,data):
        if not isinstance(data, ImageMsg): return
        # 将自定义图像消息转化为图像
