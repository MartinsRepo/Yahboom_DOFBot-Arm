...
class FaceEyeDetection:
    def __init__(self):
        ...
        if rospkg is not None:
            try:
                file_dir = os.path.join(rospkg.RosPack().get_path("dofbot_mediapipe"), "scripts", "file")
            except Exception:
                pass

        self.eyeDetect = cv.CascadeClassifier(os.path.join(file_dir, "haarcascade_eye.xml"))
        self.faceDetect = cv.CascadeClassifier(os.path.join(file_dir, "haarcascade_frontalface_default.xml"))
